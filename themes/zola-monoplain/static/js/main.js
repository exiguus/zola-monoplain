...
const appendSVGSprite = async () => {
  const usedSVGs = Array.from(document.querySelectorAll('use')).map(node => node.href.baseVal);
  ...
  const symbols = Array.from(svgSprite.querySelectorAll('symbol') || [])
    .filter(symbol => usedSVGs.includes(`#${symbol.id}`));
  if (symbols.length > 0) {
